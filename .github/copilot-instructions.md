...
1. `python3 baton.py new "short-slug"` - scaffolds `handoff/NNN-short-slug/` with
   `STATE.md`, `CONTINUE.md`, and a reconstructed `transcript.md`, and gitignores
   `handoff/`. Copilot CLI sessions are auto-detected at
   `~/.copilot/session-state/*/events.jsonl`; otherwise pass `--source PATH` or
   set `BATON_SOURCE`.
